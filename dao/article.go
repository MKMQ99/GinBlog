...
func GetCateArt(id int, pageSize int, pageNum int) ([]model.Article, int, int64) {
	...
	err := db.Where("cid =?", id).Preload("Category").Limit(pageSize).Offset((pageNum - 1) * pageSize).Find(&cateArtList).Error
...
func GetArt(pageSize int, pageNum int) ([]model.Article, int, int64) {
	...
	err = db.Select("article.id, title, img, created_at, updated_at, `desc`, comment_count, read_count, name").Limit(pageSize).Offset((pageNum - 1) * pageSize).Order("Created_At DESC").Joins("Category").Find(&articleList).Error
	// 单独计数
